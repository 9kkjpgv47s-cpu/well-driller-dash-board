...
**Base, `lithology=1`, and map-radius datasets warm independently.** Pre-warming litho alone
still leaves the first page load hitting the client's 20 s timeout on the base
`/api/wells-nearby` call (falls back to browser-side chunk loading), and the Map tab's 3-mile
dataset cold-starts separately (~55 s on a fresh prod server). Warm all three with curl before
UI assertions.
...
Resize the browser window with `wmctrl -r :ACTIVE: -e 0,0,0,<w>,<h>`. On the 1600px-wide
screen: maximized = 2xl, a 1280px window = lg, 768px = narrow/mobile-ish.
