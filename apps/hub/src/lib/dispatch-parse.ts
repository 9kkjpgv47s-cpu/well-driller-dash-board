...
export function parseDispatchEmail(raw: string): DispatchParseResult {
  ...
  if (lat !== null && lon !== null) {
    ...
    warnings.push(
      "No latitude/longitude found in the paste. The map uses a deterministic stub from the address until you paste coordinates or add geocoding.",
    );
    ...
    locationSource = "stub";
    warnings.push(
      "Could not find a clear address or coordinates. The map uses a stub from the full text — paste an address line or lat, lon.",
    );
  }
